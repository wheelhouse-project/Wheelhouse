...
6. Downloads the offline speech model if the default engine was kept (2.5 GB; this is the longest step).
7. Creates Start-menu and desktop shortcuts.
8. Reports anything worth knowing on its final page. Setup can complete and still have had to change something -- installing Parakeet because the graphics card cannot run Distil-Whisper, for example -- and those notices appear there rather than only in the setup log.
...
- **"Wheelhouse could not download a Microsoft-signed copy of the Visual C++ runtime ..."**: the install finishes, but speech may not start. Either the download failed or the downloaded file did not carry a valid Microsoft signature, and setup discarded it. Run the installer again, or install the Microsoft Visual C++ Redistributable (x64) from Microsoft, then start Wheelhouse again.
- **"Installing the Microsoft Visual C++ runtime the speech engine needs failed (code N) ..."**: the install finishes, but speech may not start. Run the installer again, or install the Microsoft Visual C++ Redistributable (x64) from Microsoft, then start Wheelhouse again. The code is Microsoft's exit code, or the word "unknown" when setup could not read one; include it in a help request.
...
- **Antivirus flags or rewrites the download**: some antivirus products quarantine downloads or alter them as they arrive. The installer verifies its own downloads -- the application, the speech model, and the AI files -- against published fingerprints and refuses anything altered (the "failed its integrity check" message); two downloads are not checked this way. uv arrives through winget or uv's own installer. The Microsoft Visual C++ runtime is downloaded from Microsoft only when the computer needs it, and setup keeps the file only when it carries a valid Microsoft signature. Add an exception for Wheelhouse, or install on a different network, then run the installer again.
