"""Distil-Whisper refuses to start when the WinRT capture cannot be built.
...
This provider is the one that could never take the WinRT path at all
before A1: winsdk was in neither its own pyproject nor the shared one,
so its factory call always fell to PortAudio. It now takes the same
path as the others, refusal included.
...
def _build_server(ws_host="localhost", ws_port=8765):
    """Construct a DistilMediumServer with winsdk simulated as absent.

    The engine and the real forwarder are stubbed because loading
    distil-medium.en needs the model files and the forwarder binds a
    websocket. Neither is reached on this path, and stubbing them keeps
    the test honest about what it measures if that ever changes.
    """
    return distil_main.DistilMediumServer(
        model_config={},
        engine_config={},
        ws_host=ws_host,
        ws_port=ws_port,
...
def test_construction_sends_the_refusal_and_exits_nonzero():
    ...
    with patch("shared_audio.capture.factory.WINRT_AUDIO_AVAILABLE", False), \
         patch.object(distil_main, "WhisperStreamingEngine"), \
         patch.object(distil_main, "WSForwarder"), \
    ...
    forwarder.send_notification.assert_called_once_with(
        "Distil-Whisper Medium (GPU)", REFUSAL, kind="startup_failed"
    )
    assert exit_info.value.code != 0


def test_the_refusal_connection_declares_the_distil_provider():
    ...
    assert kwargs["provider_name"] == "distil_medium_en"
    assert kwargs["host"] == "127.0.0.1"
    assert kwargs["port"] == 5099


def test_no_engine_is_loaded_once_the_capture_has_refused():
    """Refusing early is the point: loading distil-medium.en takes
    seconds and pins a GPU for a process that cannot transcribe."""
    with patch("shared_audio.capture.factory.WINRT_AUDIO_AVAILABLE", False), \
         patch.object(distil_main, "WhisperStreamingEngine") as engine_class, \
         patch.object(distil_main, "WSForwarder"), \
         patch("shared_stt.startup_refusal.WSForwarder") as forwarder_class:
        forwarder_class.return_value.is_connected = True
...
def test_list_devices_refuses_on_stderr_and_builds_no_forwarder(capsys):
    ...
    with patch("shared_audio.capture.factory.WINRT_AUDIO_AVAILABLE", False), \
         patch("shared_stt.startup_refusal.WSForwarder") as forwarder_class:
        code = distil_main.run_list_devices()
...
def test_list_devices_still_lists_devices_when_capture_loads():
    ...
    with patch.object(distil_main, "get_audio_provider", return_value=mic):
        code = distil_main.run_list_devices()
