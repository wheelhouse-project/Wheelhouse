...
The rule is tested per provider rather than once in a shared place. Each
provider builds its own parser, so a shared test would prove nothing
about the two that did not import it.
"""
...
def test_list_devices_needs_no_ws_port():
    ...
    args = parakeet_main.parse_provider_args(["--list-devices"])
...
def test_a_real_start_without_ws_port_is_refused():
    ...
    with pytest.raises(SystemExit) as excinfo:
        parakeet_main.parse_provider_args([])

    assert excinfo.value.code == 2


def test_a_real_start_keeps_the_port_it_was_given():
    args = parakeet_main.parse_provider_args(["--ws-port", "5001"])
